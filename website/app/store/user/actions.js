...
module.exports={
    ...
    login:async function(context){
        ...
        var type=document.head.querySelector("[name=View]").content
        console.log(type) 
    ...
    try {
        var tokens=await axios({
            ...
            data:query.stringify({
                ...
                client_id:clientId,
                code:code,
                redirect_uri:window.location.origin+window.location.pathname+`?view=${type}`
            })
        })
        window.sessionStorage.setItem('id_token',tokens.data.id_token)
        window.sessionStorage.setItem('access_token',tokens.data.access_token)
